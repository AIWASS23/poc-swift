...
extension MeshResource {
    ...
    static func generateDetailedPlane(width: Float, height: Float, vertices: (Int, Int)) throws -> MeshResource {
    ...
    static func generateCirclePlane(radius: Float, angularResolution: Int = 24, radialResolution: Int = 1, circleUV: Bool = true) throws -> MeshResource {
        var descr = MeshDescriptor()
        var meshPositions: [SIMD3<Float>] = []
        var indices: [UInt32] = []
        var textureMap: [SIMD2<Float>] = []
    ...
    static func generateArcPlane(innerRadius: Float, outerRadius: Float, startAngle: Float, endAngle: Float, angularResolution: Int = 12, radialResolution: Int = 1, circleUV: Bool = true) throws -> MeshResource {
        var descr = MeshDescriptor()
        var meshPositions: [SIMD3<Float>] = []
        var indices: [UInt32] = []
        var textureMap: [SIMD2<Float>] = []
    ...
    static func generateSquirclePlane(size: Float, p: Float = 4, angularResolution: Int = 24, radialResolution: Int  = 1, circleUV: Bool = true) throws -> MeshResource {
        var descr = MeshDescriptor()
        var meshPositions: [SIMD3<Float>] = []
        var indices: [UInt32] = []
        var textureMap: [SIMD2<Float>] = []
    ...
    static func map(input: Float, inMin: Float, inMax:Float, outMin:Float, outMax: Float) -> Float {
        return ((input - inMin) / (inMax - inMin) * (outMax - outMin)) + outMin;
    }

    static func angle2(a:simd_float2) -> Float {
        var theta = atan2f(a.y, a.x);
        if (theta < 0) { theta += .pi * 2.0; }
        return theta;
    }
    
    static func generateRoundedRectPlane(width: Float, height: Float, radius: Float, angularResolution: Int = 24, edgeXResolution: Int = 2, edgeYResolution: Int = 2, radialResolution: Int = 2, circleUV: Bool = true) throws -> MeshResource {
        var descr = MeshDescriptor()
    ...
    static func generateRoundedRectPlaneDatas(width: Float, height: Float, radius: Float, angularResolution: Int, edgeXResolution: Int, edgeYResolution: Int, radialResolution: Int, circleUV: Bool) -> (meshPositions: [SIMD3<Float>], indices: [UInt32], textureMap: [SIMD2<Float>]) {
        var meshPositions: [SIMD3<Float>] = []
        var indices: [UInt32] = []
        var textureMap: [SIMD2<Float>] = []
