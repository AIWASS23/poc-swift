...
/*
    Esse script é um utilitário para renomear um projeto Xcode. Ele usa a API FileManager para procurar e 
    substituir todas as instâncias do nome antigo do projeto pelo novo nome, tanto em nomes de arquivos e 
    pastas quanto em seus conteúdos. Ele também imprime mensagens de status no console para indicar o 
    progresso da operação de renomeação. O script possibilita passar como parametro o nome antigo e o 
    novo nome do projeto, ele vai percorrer todas as pastas dentro do diretorio onde o projeto se 
    encontra, procurando arquivos e pastas com o nome antigo e atualizando para o novo nome, além disso 
    ele também vai atualizar as referencias para o nome antigo dentro dos arquivos.
*/
