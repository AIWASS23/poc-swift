...
struct ThreeDCubeTransitionView: View {
    var body: some View {
        ThreeDCubeTransition()
    }
}

struct ThreeDCubeTransition: View {
            ...
            ForEach(data) { story in
                GeometryReader { geo in
                    ...
                    .rotation3DEffect(Angle(degrees: getAngle(xOffset: geo.frame(in: .global).minX)),
                                      ...
                                      // MARK: - anchoring View based on user drag
                                      anchor: geo.frame(in: .global).minX > 0 ? .leading : .trailing,
                                      // MARK: - perspective is ZStack Perspective
                                      perspective: 2.5
                    )
                }
            }
