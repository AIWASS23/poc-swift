...
class Toast {
    ...
    func present(title: String, symbol: String?, tint: Color = .primary, isUserInteractionEnabld: Bool = false, timing: ToastTime = .medium) {
        
        withAnimation(.snappy) {
            toasts.append(.init(title: title, symbol: symbol, tint: tint, isUserInteractionEnabld: isUserInteractionEnabld, timing: timing))
        }
    }
}
...
struct ToastItem: Identifiable {
    ...
    var symbol: String?
    var tint: Color
    var isUserInteractionEnabld: Bool
        ...
        .gesture(
            DragGesture(minimumDistance: 0)
                .onEnded({ value in
                    ...
                    guard item.isUserInteractionEnabld else { return }
