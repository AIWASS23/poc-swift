...
extension UIApplication {
    func inAppNotification<Content: View>(
        ...
        if let activeWindow = (UIApplication.shared.connectedScenes.first as? UIWindowScene)?.windows.first(where: { $0.tag == 0320 }) {
            ...
            // MARK: - Changing status into black to blend with dynamic island
            ...
            // MARK: - Creating UIView frame SwiftUI View using UIHosting Configuration
...
fileprivate struct AnimatedNotificationView<Content: View>: View {
    ...
    var body: some View {
        content
            ...
            // MARK: - Scaling Animation only ForDynamic Island Notification
    ...
    func removeNotificationView() {
        if let activeWindow = (UIApplication.shared.connectedScenes.first as? UIWindowScene)?.windows.first(where: { $0.tag == 0320 }) {
            if let view = activeWindow.viewWithTag(tag) {
                ...
                // MARK: - Resetting Once All the notifications was removed
